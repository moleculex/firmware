...
void vcp_manager(void)
{
	...
	for(;;)
	{
		...
		index = strstr(_vcp.rd_buffer.data, "cmd");
		if(index)
		{
			hexStr_to_hex(_cmd.buffer, &_vcp.rd_buffer.data[4], CMD_BUFFER_SIZE);
		...
		{
			vTaskDelay(100);
			memcpy(&_vcp.wr_buffer.data, "V0", 2);
			vcp_write(&_vcp.wr_buffer, 2);
